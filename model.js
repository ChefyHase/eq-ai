...
class Model {
  ...
  build() {
    ...
    const conv1 = tf.layers.conv2d({ filters: 16, kernelSize: 5, strides: 1, padding: 'same' }).apply(reshape);
    ...
    const conv2 = tf.layers.conv2d({ filters: 8, kernelSize: 5, strides: 1, padding: 'same' }).apply(dropout1);
