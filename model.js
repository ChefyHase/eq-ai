...
class Model {
  ...
  build() {
    ...
    const conv1 = tf.layers.conv2d({ filters: 64, kernelSize: 32, strides: 1, padding: 'same' }).apply(reshape);
    ...
    const conv2 = tf.layers.conv2d({ filters: 128, kernelSize: 5, strides: 1, padding: 'same' }).apply(dropout1);
